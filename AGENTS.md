# AGENTS.md â€” Field Tools Project Context

> ## ðŸš¨ Codex: READ THIS FILE FIRST ðŸš¨
>
...
>
> ## ðŸ”„ KEEP THIS FILE CURRENT
>
> Every time you make a change to the project, **update the corresponding section of this file in the same commit**. The file is only useful if it stays accurate. If you add a storage key, a CSS class, an animation, or change a behaviour â€” reflect it here. If you remove or invert a behaviour â€” remove or invert the note.
>
> ## âœ‹ ASK BEFORE PUSHING
>
...
>
> **Version bump rule:** decimal increments only (v1.0 â†’ v1.1 â†’ v1.2). Never whole-number jumps. Only bump immediately before a push â€” don't bump on every commit.
...
**Geolocation behaviour (current â€” do not break):**
...
- NEVER cache the lat/lng in localStorage with a TTL â€” workers may be moving and need a fresh fix every time.
...
```
Work Order Website/
â”œâ”€â”€ index.html                       # Hub (1,042 lines) â€” tool grid, dark toggle, install/bookmark popup
â”œâ”€â”€ manifest.json                    # PWA manifest, theme_color
â”œâ”€â”€ service-worker.js                # Offline cache, network-first HTML
â”œâ”€â”€ push.bat                         # Local helper: git add/commit/push
â”œâ”€â”€ data/
â”‚   â”œâ”€â”€ njfuel.json                  # ~74 NJDOT fuel stations
â”‚   â””â”€â”€ njstructures.json            # ~6,825 NJ bridge records (inline-embedded in njsearch.html too)
â”œâ”€â”€ icons/
â”‚   â”œâ”€â”€ icon-192.png                 # PWA app icon (dark navy + amber bridge arch)
â”‚   â””â”€â”€ icon-512.png                 # PWA app icon (larger)
â”œâ”€â”€ pages/
â”‚   â”œâ”€â”€ njsearch.html                # Bridge Navigator (1,440 lines)
â”‚   â”œâ”€â”€ njfuel.html                  # Fuel Station Finder (1,305 lines)
â”‚   â”œâ”€â”€ timesheet.html               # Overtime Tracker & Timesheet â€” Day/Week/Month/Settings
â”‚   â””â”€â”€ WorkOrderCloseout.html       # Work Order tool (2,949 lines)
â”œâ”€â”€ _archive/                        # old files, ignore
â”œâ”€â”€ reference/                       # data sources, ignore
â””â”€â”€ scripts/                         # transient python helper scripts (ignore)
```

**Backups (on Desktop, DO NOT TOUCH):**
- `Work Order Website - Backup 2026-05-17` â€” permanent baseline backup
- `Work Order Website - Backup 2026-05-17 (Pre-Analytics)` â€” pre-GA4 snapshot
- `Work Order Website - Backup 2026-05-18` â€” pre-large-tasklist snapshot
...
| `wo_recent` | WorkOrderCloseout | JSON array (max 5) of recent session metadata | Each rec has `{wo, str, date, fname, route, direction, mp, startDate, endDate, priority, photoKey}` â€” PHOTOS NOT INCLUDED HERE |
...
**Database:** `ft_photos` (v1) â€” defined in WorkOrderCloseout.html

**Object store:** `session_photos` â€” key is `photoKey` from wo_recent rec.
...
- `openPhotoDB()` â€” opens/caches IDB connection
- `dbPutPhotos(key, data)` â€” write
- `dbGetPhotos(key)` â€” read
- `dbDeletePhotos(key)` â€” delete on session eviction
...
:root {
  ...
  --accent: #1a56db;      /* primary blue â€” buttons, links, focus */
...
### Brand / Accent palette (NOT in CSS vars â€” hardcoded throughout)
...
| Bookmark stars / "favorite" | `#f59e0b` (amber-500) | same | Universal star color â€” DO NOT change to header accent |
...
| Coming Soon â€” Drainage | `#0891b2` (cyan) | `#67e8f9` | `card-icon-cyan`, `card-border-cyan` |
| Coming Soon â€” Milemarker | `#b45309` / `#d97706` (warm amber) | `#fcd34d` | Safety/warning theme. `card-icon-amber-soon` |
| Coming Soon â€” Emergency | `#be123c` (rose) | `#fb7185` | `card-icon-rose`, `card-border-rose` |
| Condition: Good / Open | `#22c55e` / `#16a34a` | â€” | Map markers, badges |
| Condition: Poor / Closed | `#dc2626` / `#9ca3af` | â€” | |

### Color change procedure

When asked to change the **header accent color**, sweep all hex variants together (use python script). Don't just change one â€” there are typically 80-120 references across the 4 pages in:
...
- Bookmark stars (`.bookmark-btn`, `.fuel-bookmark-btn`, `.is-bookmarked` markers) â€” always amber `#f59e0b`
- Coming Soon Milemarker tile â€” always warm amber
...
| `entering-from-hub` | tool page head inline script (reads sessionStorage) | Body slides in from left, scale 0.98â†’1 | 380ms, cubic-bezier(0.22, 1, 0.36, 1) |
| `exiting-to-hub` | tool's hub-back click + popstate | Body slides left + fades | 360ms exit, navigate at 280ms |
| `returning-from-tool` | hub head inline script + pageshow | Body slides in from right, scale 0.98â†’1 | 460ms, cubic-bezier(0.22, 1, 0.36, 1) |

### Anti-flicker pattern

Every transition animation class has the FROM state set explicitly via direct CSS properties (`opacity`, `transform`) on the selector â€” NOT just keyframes â€” so the body is guaranteed to be in the correct off-screen state before the animation actually starts. Pattern:
...
- Scroll down past 80px â†’ after 1s of sustained downscroll â†’ add `header-hidden`
- Scroll up â†’ remove `header-hidden` immediately
- Mouse near top (clientY â‰¤ 14) â†’ reveal
- Touch near top (clientY â‰¤ 30) â†’ reveal (mobile)
- Mouse hovering over topbar â†’ never hide (`mouseOverHeader` flag)

### Other key animations

- **Page fade-in (body)**: `pageFadeIn 460ms cubic-bezier(0.16, 1, 0.3, 1)` â€” default body load animation, OVERRIDDEN by `entering-from-hub` / `returning-from-tool` via `!important`
...
- **Button click glow**: `btnGlow 320ms cubic-bezier(0.16, 1, 0.3, 1)` â€” soft outer pulse via box-shadow
...
- `cubic-bezier(0.16, 1, 0.3, 1)` â€” easeOutExpo â€” luxury entrances
- `cubic-bezier(0.22, 1, 0.36, 1)` â€” easeOutQuint â€” gentle settle
- `cubic-bezier(0.65, 0, 0.35, 1)` â€” easeInOutCubic â€” symmetric navigation
- `cubic-bezier(0.4, 0, 0.2, 1)` â€” Material standard â€” exits
...
// HTML pages â†’ NETWORK-FIRST (always latest, cache as offline fallback)
// Static files (icons, JSON, manifest) â†’ CACHE-FIRST (rarely change)
// CDN libs (Leaflet, html2canvas, jsPDF, Inter font) â†’ NETWORK-FIRST with cache fallback
```
...
2. On confirmation: bump `CACHE` constant in `service-worker.js` (e.g., `ft-v1.1-2026-05-18` â†’ `ft-v1.2-2026-05-18`)
3. Bump version stamp in `index.html` footer (decimal increment only)
4. Bump "Last updated" date in `AGENTS.md`
5. `git push` â†’ GitHub Pages auto-deploys

Existing users get the new SW on their next visit; old cache is auto-deleted on activate. **localStorage and IndexedDB are NOT touched by SW cache changes** â€” user bookmarks/sessions/photos persist.
...
- Filter mode: `bookmarkFilterOn` boolean â€” when true, doSearch shows only bookmarked bridges sorted by insertion order
...
- Filter mode: `fuelBookmarkFilterOn` boolean â€” `toggleFuelBookmarkFilter()` shows bookmarked stations sorted by **Open first â†’ Hours-Vary â†’ Closed**, then by distance/name within each group
...
- Save flow: collect photos â†’ store full snapshot to IDB with `photoKey` â†’ save metadata to `wo_recent` localStorage (no photos in localStorage)
...
- Per-bridge dynamic radius: `max(50m, structure_length_m + 40m GPS buffer)` â€” accounts for NBI coords being at one approach end
- 0 matches â†’ empty state with distance to nearest bridge
- 1 match â†’ auto-opens detail
- Multiple matches â†’ bottom-sheet picker sorted by distance
- Distances always in US units (ft / mi)

### Fuel Station Finder

- Locate button always visible, re-labels to "Update My Location" after first success
- ALWAYS requests fresh location (no localStorage cache â€” users may be moving)
- Map uses `fuelMap.invalidateSize()` after location + 350ms delay (mobile layout shift fix)
- "Hide Closed" toggle on results header â€” label dynamic ("Show Closed" when hidden)
...
- **Mobile:** auto-shows bottom banner up to 2 times (`ft_install_shown` counter), then never auto-shows. Manual trigger via ðŸ“² button.
- **Desktop:** auto-shows bottom info banner up to 2 times (`ft_bookmark_shown` counter). Manual trigger via ðŸ“² button.
...
GA4 measurement ID: `G-XZJ7XL4S34` (Joe's account â€” admin-only access).

Custom events tracked:
- `find_my_bridge_tap` (njsearch) â€” pill button pressed
- `bridge_located` (njsearch) â€” GPS auto-matched to single bridge, includes `bridge_name` + `structure_number`
- `session_saved` (WorkOrderCloseout) â€” session stored to IDB, includes `work_order` + `page_count`
...
- **PWA install:** works on Chrome/Edge/Samsung (native beforeinstallprompt), iOS Safari (Shareâ†’Add to Home Screen), Firefox Android (â‹®â†’Install)
- **Service worker** registered on all pages
- **Geolocation** requires HTTPS (GitHub Pages provides)
- **IndexedDB** ~50MB+ quota, sufficient for ~20 photos Ã— 5 sessions
- **localStorage** ~5MB; we keep photos OUT of it for that reason

---

## 9.5 Naming convention â€” IMPORTANT
...
- CSS classes: `.hub-back`, `.topbar-back` â€” KEEP
- sessionStorage keys: `ft_returning_to_hub`, `ft_opening_from_hub` â€” KEEP
- JS variable names containing "hub" â€” KEEP
- URL/repo path `NJDOT-Field-Tools-Hub` â€” KEEP (renaming would break user bookmarks)
- CSS comment headers like `/* Hub-to-tool transition */` â€” fine to keep
...
- **All HTML/CSS/JS inline per file** â€” no separate .css or .js files (except service-worker.js)
- **No build step** â€” files served as-is
- **No frameworks** â€” vanilla JS only
- **CDN libs only** for Leaflet (maps), html2canvas + jsPDF (PDF export on Work Order), Inter font
- **`var` declarations everywhere** (legacy style, predates project â€” don't refactor to let/const without reason)
...
1. Always use `transform` and `opacity` only (no width/height/margin animations â€” causes layout shift)
...
5. Respect `@media (prefers-reduced-motion: reduce)` â€” already handled globally
...
- Version bumps: footer stamp + SW cache name + `AGENTS.md` "Last updated" â€” all three together in the push commit
- Decimal increments only: v1.0 â†’ v1.1 â†’ v1.2 â†’ v1.3, NEVER whole-number bumps (no v2.x)
