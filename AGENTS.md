...
- **Homepage grouped color system (2026-05-29):** dashboard groups share an accent family — Field Operations = teal/green-blue (Bridge teal, Fuel green, Milepost cyan), Documentation = purple/indigo (Work Order purple, DC-144 indigo), Time & Admin = gold (Payroll), Coming Soon = muted (`.grp-soon .card-icon` neutralizes). This is homepage-only and intentionally diverges from per-tool brand colors until Phase 2 — not a bug.
- **Agency-logo removal across tool headers (2026-05-29):** removed the base64 agency-style logo `<img>` + `.src` injector from `njsearch.html`/`njfuel.html` (`#page-logo`) and `WorkOrderCloseout.html` (`#topbar-logo`). **WorkOrder keeps `LOGO_SRC_VAR` for the PDF export (`.pdf-logo`)** — only the header img + header injector were removed. Headers now show back-pill + title only.
