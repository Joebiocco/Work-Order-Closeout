...
- **Sticky actionbar gap fix:** `#form-actionbar` top CSS transitions between `var(--topbar-h)` (topbar visible) and `0px` (topbar hidden) via `initSmartHeader()` → `updateActionbarTop()`. **Mobile actionbar (v1.21):** single `display:flex` row of 4 icon-only buttons (each `flex:1`, `min-height:44px`). Button text is wrapped in `<span class="btn-label">` and hidden via `#form-actionbar .btn-label { display:none !important }` on ≤720px. Title/divider/spacer/autosave-status still hidden. **Mobile status pill:** `#mobile-status-pill` is a `position:fixed` pill above the actionbar, hidden on desktop, shown on mobile via `@media(max-width:720px)`. Updated by `setMobileStatusPill(text, color)` called from `setAutosaveStatus()` and `updatePhotoPendingIndicator()`.
...
- **Inspector signature pad (v1.16+):** New `session.inspectorSignature` field stores a PNG data URL drawn via the signature modal (`#signature-modal`). Triggered by the `.signature-trigger` pill button next to the Inspector Name field. Canvas is `200px` tall, scales to `devicePixelRatio` for crisp strokes. Supports PointerEvents (modern) with mouse/touch fallback (older Safari). `clearSignaturePad()` uses `clearRect` (transparent, not white), `saveSignaturePad()` stores the data URL + canvas dimensions on `currentSession.inspectorSignature/Width/Height` and triggers autosave. Clearing existing signature requires a `confirm()` dialog. On export, `embedInspectorSignature(wb, ws, session)` adds the PNG via `wb.addImage` + `ws.addImage` at `tl: { col: nameCol+2, row: nameRow-1 }` (0-based). **Signature size (v1.19):** `ext.height` is fixed at 22px (~16pt) so the signature fits within the inspector-name row without bleeding into the weather rows. `ext.width` = `max(60, round(22 * canvasW / canvasH))` to preserve aspect ratio. Old drafts without the field still load — the UI guards with `!!(session.inspectorSignature)`. **Canvas init (v1.17 fix):** `openSignaturePad()` uses a double `requestAnimationFrame` (not single) before calling `initSignatureCanvas()`, ensuring the modal finishes layout before `getBoundingClientRect()` runs. `initSignatureCanvas()` also guards `if (rect.width < 10) { setTimeout(initSignatureCanvas, 80); return; }` to retry if still not painted. **Transparent background (v1.18):** canvas uses `clearRect` (not `fillRect`) so the exported PNG is transparent — signature ink does not hide the template underlines in Excel. Canvas dimensions (CSS px) are stored as `session.inspectorSignatureWidth/Height` at save time. **Modal portaling (v1.22):** both `#signature-modal` and `#export-review-modal` are routed through `prepareModalForViewport(modal)` which (a) sets `document.body.style.transform = 'none'` + `willChange = 'auto'` to clear the residual pageFadeIn transform that creates a stacking context, and (b) appends the modal to `document.documentElement` (sibling of body) so `position:fixed` is unaffected by any ancestor transform. The previous v1.21 fix appended to body — when `pageFadeIn`'s final state retained `transform:translate3d(0,0,0)` on body, the modal centered on the middle of body (full document height) instead of the viewport, dropping it below the fold when the user had scrolled.
- **Setter helpers — Excel cell writes:** `setCellValueReadable(ws,r,c,v,opts)` accepts `{wrap, shrink, top, center, middle, bottom}`. `center` sets `horizontal:'center'`, `middle` sets `vertical:'middle'`. `top` sets `vertical:'top'`. `bottom` sets `vertical:'bottom'`. Preserves all other existing alignment fields from the template. Also preserves `cell.border` (saves/restores) so template underlines survive the write. **Inspector name & date alignment (v1.22):** `DC144_BOTTOM_ALIGN_HEADER_FIELDS` is now empty (`{}`) — inspector name and date fall through to `setCellValue` (no opts) so they inherit the template's native alignment, matching projectName/contractor/contractId and sitting just above the printed underline instead of on top of it.
